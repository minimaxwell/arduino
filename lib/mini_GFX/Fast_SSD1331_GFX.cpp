...
inline void Fast_SSD1331_GFX::spiwrite(uint8_t c) {
...
void Fast_SSD1331_GFX::writeCommand(uint8_t c) {
    *rsportreg &= ~ rspin;
...
void Fast_SSD1331_GFX::writeData(uint8_t c) {
    *rsportreg |= rspin;
...
void Fast_SSD1331_GFX::goHome(void) {
  goTo(0,0);
}

void Fast_SSD1331_GFX::goTo(int x, int y) {
...
uint16_t Fast_SSD1331_GFX::Color565(uint8_t r, uint8_t g, uint8_t b) {
  uint16_t c;
  c = r >> 3;
  c <<= 6;
  c |= g >> 2;
...
/**************************************************************************/
/*
void Fast_SSD1331_GFX::fillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t fillcolor) 
{
...
void Fast_SSD1331_GFX::drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint16_t color) {	
...
void Fast_SSD1331_GFX::drawPixel(int16_t x, int16_t y, uint16_t color)
{
...
void Fast_SSD1331_GFX::pushColor(uint16_t color) {
  // setup for data
  *rsportreg |= rspin;
  *csportreg &= ~ cspin;
...
void Fast_SSD1331_GFX::begin(void) {
    // set pin directions
    pinMode(_rs, OUTPUT);
...
Fast_SSD1331_GFX::Fast_SSD1331_GFX(uint8_t cs, uint8_t rs, uint8_t sid, uint8_t sclk, uint8_t rst) : Fast_Common_GFX(TFTWIDTH, TFTHEIGHT) {
    _cs = cs;
    _rs = rs;
    _sid = sid;
    _sclk = sclk;
...
Fast_SSD1331_GFX::Fast_SSD1331_GFX(uint8_t cs, uint8_t rs, uint8_t rst) : Fast_Common_GFX(TFTWIDTH, TFTHEIGHT) {
    _cs = cs;
    _rs = rs;
    _sid = 0;
    _sclk = 0;
...
void Fast_SSD1331_GFX::copyRect(int16_t x_orig , int16_t y_orig, int16_t w, int16_t h, int16_t x_dest, int16_t y_dest){
  writeCommand(SSD1331_CMD_COPYRECT);
  writeCommand(x_orig);
  writeCommand(y_orig);
  writeCommand(w);
  writeCommand(h);
